...
{
	if (cspXMLHttp != null && !cspMozilla && !async) return cspXMLHttp;
...
function cspIntHttpServerMethod(method, args, async)
{
	...
	try {
		if (async) {
			if (cspMultipleCall) {
				...
				cspActiveXMLHttp[cspActiveXMLHttp.length] = cspXMLHttp;
				cspXMLHttp.onreadystatechange = cspProcessMultipleReq;
			} else {
				cspXMLHttp.onreadystatechange = cspProcessReq;
			}
		}
		if (cspUseGetMethod) {
			cspXMLHttp.open("GET", url+"?"+data, async);
			if (cspMozilla) {
				cspXMLHttp.send(null);
			} else {
				cspXMLHttp.send();
			}
		} else {
			cspXMLHttp.open("POST", url, async);
			cspXMLHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			cspXMLHttp.send(data);
		}
	} catch (e) {
		var err='Http object request failed, unable to process HyperEvent.';
		if (typeof cspRunServerMethodError == 'function') {
			var ret=cspRunServerMethodError(err);
			cspXMLHttp = null;
			return ret;
		}
		alert(err);
	...
	return cspProcessResponse(cspXMLHttp);
}
...
function cspHttpServerMethod(method)
{
	if (cspFindXMLHttp(false) == null) {
		var err='Unable to locate XMLHttpObject.';
		if (typeof cspRunServerMethodError == 'function')
			return cspRunServerMethodError(err);
		alert(err);
		return null;
